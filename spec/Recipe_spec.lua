...
describe("A Recipe should", function()
  ...
  describe("identify if other recipes are linkable", function()
    it("when no items are shared", function()
      assert.is_false(sp1_recipe:can_link(plate_recipe, "science-pack-1"))
    end)

    it("when items are the same type", function()
      assert.is_false(plate_recipe:can_link(plate_recipe, "iron-plate"))
    end)

    it("when items are opposite type", function()
      assert.is_true(plate_recipe:can_link(gear_recipe, "iron-plate"))
      assert.is_true(gear_recipe:can_link(plate_recipe, "iron-plate"))
    end)
  end)
end)
